...
predictions_inv_full = pd.DataFrame(
    predictions_inv_full, columns=features, index=X_test_flat.index
)
...
y_test_inv_full = pd.DataFrame(
    y_test_inv_full, columns=features, index=X_test_flat.index
)
...
plt.plot(
    X_test_inv_full.index,
    np.array(expected_pct_changes) * 100,
    label="Predicted Percentage Change",
)
