...
daily_data["BB_upper"] = daily_data["MA20"] + (
    daily_data["Close"].rolling(window=20).std() * 2
)
daily_data["BB_lower"] = daily_data["MA20"] - (
    daily_data["Close"].rolling(window=20).std() * 2
)
...
daily_data["Stochastic"] = 100 * (
    (daily_data["Close"] - low_min) / (high_max - low_min)
)
daily_data["OBV"] = (
    (np.sign(daily_data["Close"].diff()) * daily_data["Volume"]).fillna(0).cumsum()
)
...
class PositionalEncoding(Layer):
    ...
    def positional_encoding(self, position, d_model):
        ...
        cosines = tf.pad(
            cosines, [[0, 0], [0, tf.shape(sines)[1] - tf.shape(cosines)[1]]]
        )
...
if os.path.exists("./TrainedModels/AdvancedTransformer_HSI.keras"):
    ...
    for _ in range(num_layers):
        x = TransformerBlock(
            embed_dim=embed_dim, num_heads=num_heads, ff_dim=ff_dim, rate=0.3
        )(x)
    ...
    early_stop = EarlyStopping(
        monitor="val_loss", patience=30, restore_best_weights=True, mode="min"
    )
...
X_test_inv_full = pd.DataFrame(
    X_test_inv_full, columns=features, index=X_test_flat.index
)
