...
gold_data = yf.download(
    "GC=F", start=start_date, end=end_date, interval="1h", group_by="ticker"
)
silver_data = yf.download(
    "SI=F", start=start_date, end=end_date, interval="1h", group_by="ticker"
)
oil_data = yf.download(
    "CL=F", start=start_date, end=end_date, interval="1h", group_by="ticker"
)
...
merged_data["Gold_Silver_Ratio"] = (
    merged_data["Gold_Close"] / merged_data["Silver_Close"]
)
...
for commodity in ["Gold", "Silver", "Oil"]:
    # Moving Averages
    merged_data[f"{commodity}_MA10"] = (
        merged_data[f"{commodity}_Close"].rolling(window=10).mean()
    )
    merged_data[f"{commodity}_MA50"] = (
        merged_data[f"{commodity}_Close"].rolling(window=50).mean()
    )

    # Bollinger Bands
    bollinger = ta.volatility.BollingerBands(
        close=merged_data[f"{commodity}_Close"], window=20
    )
...
features = merged_data.columns.drop(
    ["Datetime", "Gold_Adj_Close", "Silver_Adj_Close", "Oil_Adj_Close"]
)
...
scaled_df = pd.DataFrame(scaled_data, columns=features, index=merged_data.index)
...
x = Conv1D(filters=128, kernel_size=3, padding="causal", activation="relu")(
    embedded_inputs
)
...
def inverse_transform_predictions(
    predictions, X_test_last, scaler, features, target_cols
):
    ...
    predictions_inv = predictions_inv_full[
        :, [features.get_loc(col) for col in target_cols]
    ]
...
predictions_inv = inverse_transform_predictions(
    predictions, X_test_last, scaler, X.columns, targets
)

# Inverse scaling for y_test
y_test_inv = inverse_transform_predictions(
    y_test, X_test_last, scaler, X.columns, targets
)
...
def unified_trading_strategy(
    predictions_inv, y_test_inv, initial_investment=10000, transaction_fee=0.01
):
