    ...
    while True:
        for symbol in symbols:
            ...
            if hourly_data.empty:
                print(
                    f"No hourly data available for {symbol}. Cannot calculate 'Volatility_hourly'."
                )
                hourly_data = None
            else:
            ...
            if data_2min.empty:
                print(
                    f"No 2-minute data available for {symbol}. Cannot calculate 'Volatility_2min'."
                )
                data_2min = None
            else:
            ...
            volatility_hourly.rename(
                columns={"Volatility": "Volatility_hourly"}, inplace=True
            )
            volatility_2min = calculate_volatility(data_2min)
            volatility_2min.rename(
                columns={"Volatility": "Volatility_2min"}, inplace=True
            )
            ...
            missing_features = [
                feat for feat in features if feat not in daily_data.columns
            ]
            if missing_features:
                print(
                    f"Missing features in daily_data for {symbol}: {missing_features}"
                )
            ...
            scaled_df = pd.DataFrame(
                scaled_data, columns=features, index=daily_data.index
            )
            ...
            model_filename = os.path.join(
                save_dir, f'model_{symbol.replace("=F", "")}.h5'
            )
            ...
            print(
                f"{symbol} prediction: {prediction_inv:.2f}, current price: {current_price:.2f}, signal: {signal}"
            )
            ...
            if signal in ["BUY", "SELL"]:
                ...
                if units == 0:
                    units = 1
                units = (
                    units if signal == "BUY" else -units
                ...
                if balance < required_margin:
                    print(
                        f"Not enough balance to execute trade for {symbol}. Available balance: {balance}, required: {required_margin}"
                    )
                    continue
